...
class ObjectDetection:
    ...
    * detectObjectsFromImage: Used to perform object detection on an image
    ...
    def __init__(self) -> None:
        ...
        self.__anchors_yolov3: List[int] = [
            10,
            13,
            16,
            30,
        ...
        self.__anchors_tiny_yolov3: List[int] = [
            10,
            14,
            23,
            27,
    ...
    def __load_image_yolo(
        self, input_image: Union[str, np.ndarray, Image.Image]
    ) -> Tuple[List[str], List[np.ndarray], torch.Tensor, torch.Tensor]:
        ...
        if type(input_image) == str:
            if os.path.isfile(input_image):
                ...
                raise ValueError(
                    f"image path '{input_image}' is not found or a valid file"
                )
    ...
    def CustomObjects(self, **kwargs):
        ...
        for karg in kwargs:
            if karg in all_objects_dict:
                ...
                raise ValueError(
                    f" object '{karg}' doesn't exist in the supported object classes"
                )
    ...
    def detectObjectsFromImage(
        ...
        if self.__model_type == "yolov3" or self.__model_type == "tiny-yolov3":
            fnames, original_imgs, input_dims, imgs = self.__load_image_yolo(
                input_image
            )
            ...
            output = get_predictions(
                pred=output.to(self.__device),
                num_classes=len(self.__classes),
                nms_confidence_level=self.__nms_score,
                objectness_confidence=self.__objectness_score,
                device=self.__device,
            )
            ...
            output[:, [1, 3]] -= (
                416 - (scaling_factor * input_dims[:, 0].view(-1, 1))
            ) / 2
            output[:, [2, 4]] -= (
                416 - (scaling_factor * input_dims[:, 1].view(-1, 1))
            ) / 2
            output[:, 1:5] /= scaling_factor

            # clip bounding box for those that extended outside the detected image.
            for idx in range(output.shape[0]):
                output[idx, [1, 3]] = torch.clamp(
                    output[idx, [1, 3]], 0.0, input_dims[idx, 0]
                )
                output[idx, [2, 4]] = torch.clamp(
                    output[idx, [2, 4]], 0.0, input_dims[idx, 1]
                )
            ...
            fnames, original_imgs, scaled_images = self.__load_image_retinanet(
                input_image
            )
            with torch.no_grad():
            ...
            if isinstance(output, torch.Tensor):
                for pred in output:
                    ...
                    original_imgs[int(pred[0].item())] = draw_bbox_and_label(
                        pred[1:5].int() if display_box else None,
                        displayed_label,
        ...
        if output_type == "file":
            if output_image_path:
                ...
                if extract_detected_objects:
                    extraction_dir = (
                        ".".join(output_image_path.split(".")[:-1]) + "-extracted"
                    )
                    ...
                    for obj_prediction in predictions_list:
                        if obj_prediction[1] >= min_probability:
                            ...
                            extracted_path = os.path.join(
                                extraction_dir,
                                ".".join(
                                    os.path.basename(output_image_path).split(".")[:-1]
                                )
                                + f"-{count}.jpg",
                            )
            ...
            raise ValueError(
                f"Invalid output_type '{output_type}'. Supported values are 'file' and 'array' "
            )

        predictions_list = [
            {
                "name": prediction[0],
                "percentage_probability": round(prediction[1] * 100, 2),
                "box_points": [
                    prediction[2]["x1"],
                    prediction[2]["y1"],
                    prediction[2]["x2"],
            ...
            if prediction[1] >= min_probability
        ]
            ...
            try:
                ...
                while input_video.isOpened():
                    ...
                    if ret == True:
                        ...
                        if counting == 1 or check_frame_interval == 0:
                            try:
                                (
                                    detected_copy,
                                    output_objects_array,
                                ) = self.__detector.detectObjectsFromImage(
                                    input_image=frame,
                                    output_type="array",
                        ...
                        for eachItem in output_objects_array:
                            ...
                            try:
                                output_objects_count[eachItemName] = (
                        ...
                        if per_minute_function != None:
                            if (
                                counting != 1
                                and (counting % (frames_per_second * 60)) == 0
                            ):
                                ...
                                if return_detected_frame == True:
                                    per_minute_function(
                                        int(counting / (frames_per_second * 60)),
                                        this_minute_output_object_array,
                                        this_minute_counting_array,
                                        this_minute_counting,
                                        detected_copy,
                ...
                if video_complete_function != None:
                    ...
                    for eachCountingDict in this_video_counting_array:
                        for eachItem in eachCountingDict:
                            try:
                                this_video_counting[eachItem] = (
                                    this_video_counting[eachItem]
                                    + eachCountingDict[eachItem]
                                )
                            except:
                                this_video_counting[eachItem] = eachCountingDict[
                                    eachItem
                                ]
